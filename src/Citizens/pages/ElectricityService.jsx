...
const ISSUE_TYPES = ['Power outage', 'Voltage fluctuation', 'Transformer fault', 'Exposed / dangerous wires', 'Street light not working', 'Bill dispute', 'New connection request', 'Meter issue'];

const INITIAL_FORM = {
  name: '', phone: '', consumerNo: '', address: '', issueType: '', description: '',
  proofImage: null, previewUrl: null, coordinates: null,
};
...
export default function ElectricityService() {
  ...
  const fetchComplaints = async () => {
    ...
    try { setComplaints(await getComplaintsByDepartment('Electricity')); }
    catch (err) { console.error('[ElectricityService] fetch error:', err); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let imageUrl = null;
      if (formData.proofImage) imageUrl = await uploadImageToImageKit(formData.proofImage, '/complaints/electricity');
      ...
      await createComplaint({
        ...
        issueType: formData.issueType, description: formData.description.trim(),
        address: formData.address.trim(), coordinates: formData.coordinates || null,
      ...
      console.error('[ElectricityService] submit error:', err);
        ...
        <div className="service-two-col">
          <section className="service-left-col">
            <div className="service-info-card card">
              ...
              <p className="service-form-intro">Emergency issues are prioritised and attended within 4 hours. You earn <strong>+10 Pragati Points</strong> for each valid submission.</p>
              ...
              <form className="service-form" onSubmit={handleSubmit} noValidate>
                ...
                <div className="form-group">
                  <label htmlFor="e-name" className="form-label">Full Name</label>
                  <input id="e-name" type="text" className="form-input" placeholder="Sunita Sharma" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                </div>
                <div className="form-group">
                  <label htmlFor="e-phone" className="form-label">Mobile Number</label>
                  <input id="e-phone" type="tel" className="form-input" placeholder="9876543210" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                </div>
                <div className="form-group">
                  <label htmlFor="e-consumer" className="form-label">Consumer / Meter Number</label>
                  <input id="e-consumer" type="text" className="form-input" placeholder="MH-12345678" value={formData.consumerNo} onChange={e => setFormData({ ...formData, consumerNo: e.target.value })} />
                </div>
                <div className="form-group">
                  <label htmlFor="e-address" className="form-label">Address / Location *</label>
                  <input id="e-address" type="text" className="form-input" placeholder="Street, Ward, Sector" value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })} required />
                </div>
                <div className="form-group">
                  <label htmlFor="e-issue" className="form-label">Issue Type *</label>
                  <select id="e-issue" className="form-select" value={formData.issueType} onChange={e => setFormData({ ...formData, issueType: e.target.value })} required>
                    <option value="">-- Select Issue --</option>
                    {ISSUE_TYPES.map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label htmlFor="e-desc" className="form-label">Description *</label>
                  <textarea id="e-desc" className="form-textarea" rows={4} placeholder="Describe the issue in detail..." value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} required />
                </div>
                <div className="form-group">
                  <label className="form-label">Upload Photo (optional)</label>
                  <label className={`file-upload-area ${formData.proofImage ? 'uploaded' : ''}`} htmlFor="e-photo" style={{ cursor: 'pointer' }}>
                    {formData.previewUrl ? (
                    ...
                    <input id="e-photo" type="file" accept="image/*" style={{ display: 'none' }} onChange={handleImageChange} />
                  </label>
                </div>
                <div className="form-group">
                ...
                <MapContainer center={[17.6868, 75.9042]} zoom={12} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
                  ...
                  {complaints.filter(c => c.coordinates?.lat).map(c => (
                    <Marker key={c.id} position={[c.coordinates.lat, c.coordinates.lng]}>
                      <Popup><strong>{c.issueType || 'Electricity'}</strong><br />{c.address}<br />Status: <strong>{c.status}</strong></Popup>
                    </Marker>
                  ))}
                </MapContainer>
              </div>
