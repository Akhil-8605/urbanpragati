...
export default function WaterDept() {
  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      try {
        setLoading(true);
        const wData = await getWorkersByDepartment('Water');
        setWorkersList(wData);
        
        unsub = listenComplaintsByDepartment('Water', (data) => {
    ...
    if (userData.role === 'admin' && userData.department && userData.department !== 'Water') {
      navigate('/admin');
    } else {
      fetchInitial();
    }
  ...
  // Generate chart data
  const statusData = [
    { name: 'Pending', value: complaintsList.filter(c => c.status === 'Pending').length },
    { name: 'In Progress', value: complaintsList.filter(c => c.status === 'In Progress').length },
    { name: 'Resolved', value: complaintsList.filter(c => c.status === 'Resolved').length },
  ].filter(d => d.value > 0);
  
  const workerWorkload = workersList.map(w => ({
    name: w.displayName || w.name || w.email?.split('@')[0] || 'Unknown',
    assigned: complaintsList.filter(c => c.workerId === w.id && c.status !== 'Resolved').length,
    completed: complaintsList.filter(c => c.workerId === w.id && c.status === 'Resolved').length,
  }));
  
  const COLORS = ['#e65100', '#2563eb', '#10b981'];
      ...
      <div className="admin-main">
        ...
        <main className="admin-content">
          <div className="admin-page-header">
            <div>
              <p className="breadcrumb">Admin &rsaquo; Departments &rsaquo; Water</p>
              <h1 className="admin-page-title">Water Department</h1>
            </div>
          ...
          {!loading && complaintsList.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
              <div style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0, 0, 0, 0.06)' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '1rem', color: '#333' }}>Status Distribution</h3>
              ...
              <div style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0, 0, 0, 0.06)' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '1rem', color: '#333' }}>Worker Workload</h3>
                <div style={{ width: '100%', height: 250 }}>
                  <ResponsiveContainer>
                    <BarChart data={workerWorkload} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                      <XAxis dataKey="name" tick={{ fontSize: 12 }} />
                      <YAxis tick={{ fontSize: 12 }} />
                      <Tooltip cursor={{ fill: '#f4f6fb' }} />
                      <Legend />
                      <Bar dataKey="assigned" fill="#2563eb" radius={[4, 4, 0, 0]} />
              ...
              {active ? (
                <>
                  <div className="dept-detail-header">
                    <div>
                      <span className="cdc-id">{active.id}</span>
                      <h2 className="dept-detail-title">{active.title}</h2>
                    </div>
                    <span className={`chip chip-${active.status === 'Pending' ? 'pending' : active.status === 'In Progress' ? 'inprogress' : 'resolved'}`}>
                      {active.status}
                    </span>
                  </div>
                  ...
                  {active.imageUrl ? (
                    ...
                    <div className="dept-img-placeholder" aria-label="Complaint photo placeholder">
                  ...
                  <div className="dept-timeline-section">
                    ...
                    <ol className="dept-timeline" aria-label="Complaint timeline">
                      <li className="dept-timeline-item dept-timeline-item--done">
                        ...
                        <div className="dept-timeline-content">
                          <span className="dept-timeline-step">Complaint Registered</span>
                          <span className="dept-timeline-date">{active.date}</span>
                        </div>
                      </li>
                      <li className={`dept-timeline-item ${active.status !== 'Pending' ? 'dept-timeline-item--done' : ''}`}>
                        <span className="dept-timeline-dot" aria-hidden="true" />
                        <div className="dept-timeline-content">
                          <span className="dept-timeline-step">Worker Assigned</span>
                          <span className="dept-timeline-date">{active.status !== 'Pending' ? '✓' : '—'}</span>
                        </div>
                      </li>
                      <li className={`dept-timeline-item ${active.status === 'Resolved' ? 'dept-timeline-item--done' : ''}`}>
                        <span className="dept-timeline-dot" aria-hidden="true" />
                        <div className="dept-timeline-content">
                          <span className="dept-timeline-step">Resolved</span>
                          <span className="dept-timeline-date">{active.status === 'Resolved' ? '✓' : '—'}</span>
                        </div>
                      </li>
                    </ol>
                  </div>
